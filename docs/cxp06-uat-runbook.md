...
**REVALIDATION PARTIAL — corrective rerun required, August 26, 2026.** The dataset-scoped adaptive-worker redesign passed peak runtime revalidation: Case 1 completed in 16m 00.749s with a longest invocation of 204.372s and no timeout. The same evidence set exposed a health-fault decorator regression and missing durable failure-audit rows in continuation-owned fault/recovery cases. See `docs/cxp06-hosted-uat-revalidation-2026-08-26.md`. Fix those blockers and repeat Cases 03, 04, 04.1, 05.3, and 05.4 before promotion.
...
Run `getCxp06UatPipelineStatus()` to emit a sanitized `CXP06_STATUS` record containing the actual continuation-trigger presence, last completed backup dataset, last completed commit dataset, and final `backupCleanupStatus`. The cleanup field is `null` before finalization and `DELETED` or `PENDING` afterward. Expected terminal state is `COMPLETE`; `FAILED` retains a bounded error code. If status is nonterminal but `continuationScheduled` is `false`, rerun the same main scenario entrypoint once. It preserves the checkpoint and installs one 60-second recovery trigger; it does not replay preparation. Rerunning the same scenario then resumes backup discovery, raw replacement from the saved cursor, or final verification from durable state. Do not start a different scenario while one is active.
...
Continuation-owned terminal failures report `failureAuditStatus: PENDING` until the matching `RUN_LOG` and `ERROR_LOG` rows have been persisted. While pending, the one-time continuation retries only audit persistence and never replays backup or commit work. `failureAuditStatus: RECORDED` and `continuationScheduled: false` are required before accepting terminal failure evidence. `lastAuditErrorCode` reports a bounded reporting failure without replacing the original `lastErrorCode`.
...
4. Separately inject post-success backup deletion failure with `CASE5_CLEANUP_FAILURE`. Confirm the successful raw result remains active, `getCxp06UatPipelineStatus()` durably reports `backupCleanupStatus: PENDING`, exactly five backup sheets remain, and the next cleanly controlled run removes the leftover group.
