...
Implementation-level decisions made by CXP-00 through CXP-14, including the later GMT-extraction clarification, are recorded in the [decision log](decision-log.md). CXP-14 repository decisions through DEC-069 are implemented: hosted UAT steps orchestrate predecessor work, retarget UAT Inbox fixture folders from Script Properties, and record allowlisted evidence internally, while production cutover remains approval-bound.
