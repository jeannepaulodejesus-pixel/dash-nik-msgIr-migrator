...
- **Approved decisions:**
  ...
  5. Normalize empty/whitespace-only cells and the eight allowlisted spreadsheet error tokens to null without defaults; key fields may not be blank. Reject unknown trimmed `#...` values. `NA` remains ordinary text, not a null sentinel. (Superseded by DEC-049.)
