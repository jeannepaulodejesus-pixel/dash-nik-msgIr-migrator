...
CXP-11 local tests drive the whole parity run through injected export, target, results, baseline, ledger, lock, and trigger ports, so no Drive folder or spreadsheet ID is needed. Node cannot recalculate a legacy Excel control, execute Google Sheets formulas, or prove hosted trigger timing and quota behavior. An authorized operator must separately execute `docs/cxp11-uat-runbook.md` in DEV/UAT with an operator-recalculated export bundle whose `sourceBundleFingerprint` matches a successful `FILE_LEDGER` entry. Hosted helpers are documented in `docs/cxp11-uat-harness.md`; record results with `docs/cxp11-hosted-uat-results-template.md` and the run summary with `docs/cxp11-parity-report-template.md`.
