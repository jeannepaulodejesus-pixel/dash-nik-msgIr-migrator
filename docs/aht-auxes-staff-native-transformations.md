...
The hosted runner divides installation into 74 retry-safe steps: one schema preflight, then capacity, clear, header, formula-anchor, and raw-copy operations for each calculation sheet, plus Staff summary headers/formulas. It saves the next step in the `CXP08_AHT_AUXES_STAFF_INSTALL_STATE` Script Property after every successful operation and stops normal work after four minutes. A one-second time-driven continuation resumes the saved cursor. A seven-minute safety trigger guards hard cutoffs. A script lock prevents overlapping runners.
