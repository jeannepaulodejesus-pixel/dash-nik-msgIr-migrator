...
- **Architecture:** `CommitService.createOperations()` supplies the five CXP-04 callbacks for staging, validation, commit, recalculation, and health. It reuses the existing state machine and script lock; no second orchestrator or lock exists.
...
- **Focused tests:** `npm run test:cxp06` passes 23/23 tests.
...
- **Hosted UAT:** Peak Case 01 has completed successfully within the current Apps Script limit. Hosted topology evidence remains pending for the four controlled Case 05 recovery shapes, as do complete Case 03/04 fault evidence and reader-isolation observation. Follow `docs/cxp06-uat-runbook.md` in a clean non-production workbook before promotion.
...
- **Known limitation:** Formula-like text beginning with `=` is rejected during persisted-stage formula validation because Apps Script `setValues` documents formula interpretation. Literal acceptance would require a separately verified RAW-input or escaping contract.
