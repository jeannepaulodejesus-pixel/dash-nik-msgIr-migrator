...
- **Hosted UAT:** Revalidation remains pending. August 24 evidence is bound to its recorded source state. August 25 runs first timed out after preparation and then timed out again inside the isolated commit after entering it with about 323 seconds remaining. That evidence triggered the one-dataset-per-invocation backup journal. A later August 25 run completed all five backups and one raw dataset, then recorded `FAILED` with `INGESTION_LOCK_TIMEOUT` because the 270-second watchdog re-entered a live commit loop and the retryable timeout deleted the continuation; DEC-044 corrects the watchdog delay, the reserved-step budget, and contention handling. A subsequent run then recorded `MIGRATION_COMMIT_FAILED` wrapping `MIGRATION_BACKUP_FAILED` with verified rollback after Handled was replaced, because `commitStep` compared every backup to current raw; DEC-045 compares only unreplaced datasets. Repeat Case 01 and the fault/recovery matrix against that revision before promotion, reconciling any partially committed target workbook first. Hosted topology evidence remains pending until that rerun is captured.
